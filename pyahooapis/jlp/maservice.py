...
class MAService(Service):
	...
	def get_result_set(self, sentence, results=RESULT_MA, response=None, result_filter=None,
		...
		if result_filter is not None:
			params['filter'] = self._binary2param('|', result_filter, FILTERS)
	...
	def get_ma_result(self, sentence, response=None, result_filter=None, json=False):
		if json:
			return self.get_result_set(sentence=sentence, response=response, filter=result_filter, json=json)
		else:
			return self.get_result_set(sentence=sentence, response=response, filter=result_filter, json=json).ma_result
	
	def get_uniq_result(self, sentence, response=None, result_filter=None, json=False):
		if json:
			return self.get_result_set(sentence=sentence, results=RESULT_UNIQ, response=response, filter=result_filter, json=json)
		else:
			return self.get_result_set(sentence=sentence, results=RESULT_UNIQ, response=response, filter=result_filter, json=json).uniq_result
